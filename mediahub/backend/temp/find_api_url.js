async function findApiUrl() {
  const root = 'https://cobalt.meowing.de';
  try {
    ...
    for (const match of matches) {
      ...
      // Also search for references to custom header/API strings
      if (jsCode.includes('api') || jsCode.includes('processing')) {
        console.log(`File ${match} contains references to API/processing.`);
      }
    }
  } catch (err) {
